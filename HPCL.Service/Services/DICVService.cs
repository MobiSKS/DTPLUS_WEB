...
namespace HPCL.Service.Services
{
    public class DICVService: IDICVService
    {
        ...
        public async Task<DICVCustomerAdvancedSearchModel> DICVCustomerAdvancedSearch()
        {
            DICVCustomerAdvancedSearchModel model = new DICVCustomerAdvancedSearchModel();

            model.lstCustomerType.AddRange(await _commonActionService.GetOtherCustomerTypeDropdown());
            model.CustomerType = 953;
        ...
        public async Task<GetDICVAdvancedSearchResponse> GetJCBAdvancedSearch(string str)
        {
            JArray objs = JArray.Parse(JsonConvert.DeserializeObject(str).ToString());
            List<DICVCustomerAdvancedSearchRequest> arrs = objs.ToObject<List<DICVCustomerAdvancedSearchRequest>>();
            ...
            var insertServiceBody = new DICVCustomerAdvancedSearchRequest
            {
                UserId = _httpContextAccessor.HttpContext.Session.GetString("UserId"),
                UserAgent = CommonBase.useragent,
                UserIp = _httpContextAccessor.HttpContext.Session.GetString("IpAddress"),
            ...
            StringContent content = new StringContent(JsonConvert.SerializeObject(insertServiceBody), Encoding.UTF8, "application/json");
            var response = await _requestService.CommonRequestService(content, WebApiUrl.getDicvAdvancedSearch);

            JObject obj = JObject.Parse(JsonConvert.DeserializeObject(response).ToString());
            GetDICVAdvancedSearchResponse result = obj.ToObject<GetDICVAdvancedSearchResponse>();
